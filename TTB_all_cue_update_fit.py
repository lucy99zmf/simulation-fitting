	...
	def TTB_all_fit(para, symm_updates=1, verbose=0):
		...
		for cycle in range(50):
			...
			for trial in range(len(xs)):
				...
				abs_qs = list(np.reshape(present_qs, len(present_cues)))
				used_cues = []

				for i in range(math.ceil(k) if k%1 > np.random.uniform(0, 1) else math.floor(k)):
					if np.shape(present_cues)[0]-1:
						cue = np.argmax(abs_qs[1:])+1; 
						used_cues.append(present_cues.pop(cue)); 
						abs_qs.pop(cue)
					else:
						pass
				...
				y_sum = np.matmul(qs, x_new)	#choice prob of A1 stochastic decision rule
		...
		print(np.nanmean(all_errors))

		return np.nanmean(all_errors)

	##finding best fit 
	x0 = [.05, 1, 2]
	bnds = ((0,.2), (0, 10), (0,4))
	res = sp.optimize.minimize(TTB_all_fit, x0, method = 'Nelder-Mead') 
	print(res)
	# para = res.x 
	# print(para)
...
# 	for k in beta:
# 		print(i,k)
# 		row.append(TTB_all_fit([i, k, cue]))
# 	error.append(row)

# ax = sns.heatmap(np.array(error))
